...
#' @noRd
check_response_status_code <- function(response) {
  status_code <- status_code(response)

  if (status_code == 401) stop("Error 401. The error might be caused by your access token. Please verify that it is correctly defined.")

  if (status_code == 403) stop("Error 403. This request is forbidden. Please check your role in the gitlab project and the corresponding permissions.")

  if (status_code == 404) stop("Error 404. The repository might not exist or might be a private repository for which your access is not granted.")
