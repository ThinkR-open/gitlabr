...
#' # Access repository
#' # _All files
#' gl_repository(project = "test_project_id")
#' # _All contributors
#' gl_repository(project = "test_project_id", "contributors")
#' # _List files
#' gl_list_files(project = "test_project_id")
#' # _Get content of one file
#' gl_get_file(project = "test_project_id", file_path = "README.md")
#' # _Test if file exists
#' gl_file_exists(project = "test_project_id", file_path = "README.md", ref = "main")
#' }
...
gl_push_file <- function(project,
  ...
  if (!exists || overwrite) {
    gitlab(
      req = gl_proj_req(project = project, c("repository", "files", file_path), ...),
