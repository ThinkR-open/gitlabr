...
test_that("getting issues works", {
  ...
  expect_is(my_project(gl_list_issues, 2), "data.frame")
  ...
  expect_is(gl_list_issues(issue_id = 2, gitlab_con = my_project), "data.frame")
...
test_that("editing issues works", {
  ...
  expect_true(my_gitlab(gl_list_issues, "testor", 2)$state == "reopened")
  
  ## edit its description
  my_gitlab(gl_edit_issue, 2, "testor", description = "This is a test")
  expect_true(my_gitlab(gl_list_issues, "testor", 2)$description == "This is a test")
  ...
  expect_true(my_gitlab(gl_list_issues, "testor", 2)$state == "closed")
  
  ## using gl_project_connection
  my_project(gl_reopen_issue, 2)
  expect_true(my_project(gl_list_issues, 2)$state == "reopened")
  my_project(gl_close_issue, 2)
  expect_true(my_project(gl_list_issues, 2)$state == "closed")
  
  ## using function idiom
  gl_reopen_issue(issue_id = 2, gitlab_con = my_project)
  expect_true(gl_list_issues(issue_id = 2, gitlab_con = my_project)$state == "reopened")
  gl_close_issue(issue_id = 2, gitlab_con = my_project)
  expect_true(gl_list_issues(issue_id = 2, gitlab_con = my_project)$state == "closed")
